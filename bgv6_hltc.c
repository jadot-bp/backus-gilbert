...
int main(int argc, char *argv[]){
    ...
    */
    ...
    for (int w=0; w<=Ns; w++){    
        ...
        /* Construct kernel sampling vector */

        mpfr_t KSamp[t2-t1];

        mpfr_t work;               //Work variable
        mpfr_t temp;               //Temporary storage variable
                                   
        mpfr_init2(work,prec);
        mpfr_init2(temp,prec);

        for (int i=0; i<t2-t1; i++){
            mpfr_init2(KSamp[i],prec);
        ...
        for (int i=0; i<t2-t1; i++){
            mpfr_init_set_ld(KConst[i],IntKFunc(w0,wmin,tau[i]),MPFR_RNDF);
        }

        mpfr_t AvgCoeff[t2-t1];
        
        /* Calculating AvgCoeff = K^-1 x D + K^-1 x C [(1 - C^T x K^-1 x D)/ C^T x K^-1 x C] */
            
        mpfr_t KInvC[t2-t1];
        mpfr_t KInvD[t2-t1];

        
        /* Calculate K^-1 x C and K^1 x D*/
        for (int i=0; i<t2-t1; i++){
            mpfr_init_set_ld(KInvC[i],0.0,MPFR_RNDF);
            mpfr_init_set_ld(KInvD[i],0.0,MPFR_RNDF);
 
            for (int j=0; j<t2-t1; j++){
                mpfr_mul(work,KInverse[i][j],KConst[j],MPFR_RNDF);
                mpfr_add(KInvC[i],KInvC[i],work,MPFR_RNDF);

                mpfr_mul(work,KInverse[i][j],KSamp[j],MPFR_RNDF);
                mpfr_add(KInvD[i],KInvD[i],work,MPFR_RNDF);
            }
        }
 
        /* Calculate norms */

        mpfr_t normC;   // C^T x K^-1 x C
        mpfr_t normD;   // C^T x K^-1 x D

        mpfr_init_set_ld(normC,0.0,MPFR_RNDF);
        mpfr_init_set_ld(normD,0.0,MPFR_RNDF);
   
        for (int i=0; i<t2-t1; i++){
            mpfr_mul(work,KInvC[i],KConst[i],MPFR_RNDF);
            mpfr_add(normC,normC,work,MPFR_RNDF);

            mpfr_mul(work,KInvD[i],KConst[i],MPFR_RNDF);
            mpfr_add(normD,normD,work,MPFR_RNDF); 
        }

        /* Calculate norm fraction (1-C^T x K^-1 x D)/ C^T x K^-1 x C */

        mpfr_t normf;
        mpfr_init2(normf,prec);

        mpfr_d_sub(normf,1.0,normD,MPFR_RNDF);
        mpfr_div(normf,normf,normC,MPFR_RNDF);

        for (int i=0; i<t2-t1; i++){
            mpfr_init_set_ld(AvgCoeff[i],0.0,MPFR_RNDN);

            mpfr_add(AvgCoeff[i],AvgCoeff[i],KInvD[i],MPFR_RNDF);

            mpfr_mul(work,normf,KInvC[i],MPFR_RNDF);
            mpfr_add(AvgCoeff[i],AvgCoeff[i],work,MPFR_RNDF);
        }
